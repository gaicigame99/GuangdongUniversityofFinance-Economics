...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            exit()
        if event.type == pygame.MOUSEBUTTONUP\
                and (480 - start_meum.button_width)/2 < mouse_x < (480 - start_meum.button_width)/2 + start_meum.button_width\
...
while True:
    ...
    if whether_show_start == True:
        ...
        for i in range(random.randint(1, 3)):
            block = Block(random.randint(0, 480 - 60), random.randint(680, 720), random.randint(0, 3), random.randint(difficulty_a, difficulty_b))
            block_list.append(block)
            difficulty_a += 1
            difficulty_b += 3
